...
Utworzono program testujacy, przykladowe wywolania (testy) mozna poprzez make test

Niestety nie udalo mi sie ladniej ogarnac przekazania linii z testami.
Probowalem m in poprzez read line < [nazwa_testu].txt
Po nieudanych probach postanowilem wkleic gotowe wywolania maina jako testy
Dlatego zostawiam komentarz w make, aby ponizej pewnego miejsca nie scrollowac - wszystko co wazne jest u gory.

Do szybkiego i wygodnego porownania czasow w make jest osobny test
    ...
    for(int i=2;i<argc;){
        ...
        if(strcmp(argv[i],"create_table")==0){
            ...
            while(counter+1<argc && strcmp(argv[counter+1],"create_table")!=0
                    ...
                    ///fprintf(result, "New block created from %s and %s added\n",file1,file2); /// dla czytelnosci wrzucam do pliku tylko ogolna informacje ile par
                }
